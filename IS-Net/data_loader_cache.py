...
def im_preprocess(im,size):
    ...
    im_tensor = torch.as_tensor(im.copy(), dtype=torch.float32)
    im_tensor = torch.transpose(torch.transpose(im_tensor,1,2),0,1)
    if(len(size)<2):
        return im_tensor, im.shape[0:2]
    else:
        im_tensor = torch.unsqueeze(im_tensor,0)
        im_tensor = F.interpolate(im_tensor, size, mode="bilinear",align_corners=True)
...
def gt_preprocess(gt,size):
    ...
    gt_tensor = torch.unsqueeze(torch.as_tensor(gt, dtype=torch.uint8),0)

    if(len(size)<2):
        return gt_tensor.type(torch.uint8), gt.shape[0:2]
    else:
        gt_tensor = torch.unsqueeze(torch.as_tensor(gt_tensor, dtype=torch.float32),0)
        gt_tensor = F.interpolate(gt_tensor, size, mode="bilinear",align_corners=True)
