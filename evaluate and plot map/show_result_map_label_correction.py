...
if __name__ == '__main__':
    ...
    for img_name in name:
        ...
        # sod
        sod_path = r"E:/GID_experiment/allocate_data_492/test/sod_isnet_4_2000_raw" + os.sep + mask_name[:-4] + '.png'
        sod = np.array(cv2.imread(sod_path, -1))
        
        # sod+crf
        sod_crf_path = r"E:\GID_experiment\allocate_data_492\test\sod_isnet_4_2000_raw_crf"  + os.sep + mask_name
        sod_crf = np.array(cv2.imread(sod_crf_path, -1))
        
        img = [img_path, true_mask, raw_mask, sod, sod_crf]
        name = ['原始影像','真实标签', '初始标签', '校正标签 (SOD)','校正标签 (SOD+CRF)']
    
        plot_maps(images=img, images_name=name, row_number=1, fontsize=22)
    
        plt.savefig(r'D:\硕士论文\gaofen_picture_0309\label_correction\{}.png'.format(img_name[:-4]), bbox_inches='tight')
