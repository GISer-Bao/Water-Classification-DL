...
class RSU(nn.Module):
    def __init__(self, height: int, in_ch: int, mid_ch: int, out_ch: int):
        ...
        self.out = ConvBNReLU(out_ch, out_ch)
...
class U2Net(nn.Module):
    def __init__(self, cfg: dict, out_ch: int = 1):
        ...
        for c in cfg["encode"]:
            ...
            if c[5] is True:
                ...
                side_list.append(ConvBNReLU(c[3], out_ch))
        self.encode_modules = nn.ModuleList(encode_list)
    ...
    def forward(self, x: torch.Tensor) -> Union[torch.Tensor, List[torch.Tensor]]:
        ...
        x = self.out_conv(side_outputs[0])
        # x = side_outputs[0]
...
def u2net_full(in_ch: int = 3,out_ch: int = 1):
    cfg = {
        ...
        # height, in_ch, mid_ch, out_ch, RSU4F, side
        "decode": [[4, 512, 256, 512, True, True],   # De5
                    [4, 512, 128, 256, False, True],  # De4
                    [5, 256, 64, 128, False, True],    # De3
                    [6, 128, 32, 64, False, True],     # De2
