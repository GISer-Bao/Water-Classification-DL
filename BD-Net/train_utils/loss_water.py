...
# logits: 卷积后的输出结果，[batch_size, class, x_size, y_size]
# labels: 标签，[batch_size, x_size, y_size]
# n_min: batch_size * x_size * y_size // 16
# thresh: 0.7
...
# n_min: batch_size * x_size * y_size // 16
# thresh: 0.7
# radius: Recommended 1. Larger radius generates thicker edges
class ECELoss(nn.Module):
    def __init__(self, thresh, n_min, n_classes=2, alpha=0.01, radius=1, beta=0.5, ignore_lb=255, mode='ohem', *args, **kwargs):
...
class EdgeLoss(nn.Module):
    def __init__(self, n_classes=2, radius=1, alpha=0.01):
...
# predict: 卷积后的输出结果，[batch_size, class, x_size, y_size]
# target: 标签，[batch_size, x_size, y_size]
...
if __name__ == '__main__':
    ...
    net1 = nn.Sequential(nn.Conv2d(3, 2, kernel_size=3, stride=2, padding=1))
    net1.cuda()
    net1.train()
    net2 = nn.Sequential(nn.Conv2d(3, 2, kernel_size=3, stride=2, padding=1))
    net2.cuda()
    net2.train()

    with torch.no_grad():
        inten = torch.randn(16, 3, 20, 20).cuda()
        lbs = torch.randint(0, 2, [16, 20, 20]).cuda()
        lbs[1, 10, 10] = 255
